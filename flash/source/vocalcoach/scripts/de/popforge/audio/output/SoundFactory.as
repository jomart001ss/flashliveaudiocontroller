...
package de.popforge.audio.output
{
	...
	public class SoundFactory
	{
		...
		static private var s: ByteArray = null;
		...
		static private var loader:Loader=null;
		static private var onComplete:Function=null;
		/**
		 * Creates a flash.media.Sound object from dynamic audio material
		 * 
		...
		static public function fromByteArray( bytes: ByteArray, channels: uint, bits: uint, rate: uint, onComplete: Function ): void
		{
			...
			{
				if (testSound==null) testSound=Sound( new ( loader.contentLoaderInfo.applicationDomain.getDefinition( 'SoundItem' ) as Class )() ) ;
			...
			*/
			...
			loader = new Loader();
			loader.contentLoaderInfo.addEventListener( Event.COMPLETE, onSWFLoaded, false, 0, true); // use weak references 
			loader.contentLoaderInfo.addEventListener( Event.UNLOAD, onSWFUnloaded, false, 0, true); // use weak references 
			loader.loadBytes( swf );
			
			var time4:Number=getTimer();
			trace("soundfactory="+(time1-time0)+" "+(time2-time1)+" "+(time3-time2)+" "+(time4-time3));
		}
		
		static public var testSound:Sound=null;
		static public var sounds:Array=new Array();
		static public function onSWFLoaded(ev:Event):void {
			trace("onSWFLoaded");
			//if (testSound==null) testSound=Sound( new ( ev.target.applicationDomain.getDefinition( 'SoundItem' ) as Class )() ) ;
			//onComplete(testSound);
			var s:Sound=Sound( new ( loader.contentLoaderInfo.applicationDomain.getDefinition( 'SoundItem' ) as Class )() );
			//sounds.push(s);
			onComplete( s );
			loader.removeEventListener( Event.COMPLETE, onSWFLoaded );
			try { loader.unload(); loader.close(); } catch (err:Error) {}
			//loader=null;
			//ev=null;
		}
		static public function onSWFUnloaded(ev:Event):void {
			trace("onSWFUnloaded");
		}
	}
}